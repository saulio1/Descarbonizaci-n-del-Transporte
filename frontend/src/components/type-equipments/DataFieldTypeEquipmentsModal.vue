<template>
  <Dialog
    v-model:visible="typeEquipmentsStore.isDialogDataFieldOpen"
    :style="{ width: '30rem' }"
    :header="
      typeEquipmentsStore.dialogDataFieldMode === 'create'
        ...
        <div class="flex flex-col gap-1">
          ...
          <span
            v-if="
              ...
              typeEquipmentsStore.selectedDataField?.name !== name.trim()
            "
            class="text-red-400 dark:text-red-400/80 text-sm"
          >
...
const initDataField = () => {
  const df = typeEquipmentsStore.selectedDataField
  if (df) {
    name.value = df.name
...
watch(
  () => typeEquipmentsStore.selectedDataField,
  () => {
    initDataField()
  },
)
...
const close = () => {
  ...
  initDataField()
  typeEquipmentsStore.closeDialogDataField()
}

function onDialogVisibilityChange(val: boolean) {
  typeEquipmentsStore.isDialogDataFieldOpen = val
  if (!val) {
    close()
  }
}
...
const validateForm = () => {
  ...
  const errors = {
    ...
    nameExists:
      ...
      typeEquipmentsStore.selectedDataField?.name !== name.value.trim(),
    options:
...
const save = async () => {
  ...
  switch (typeEquipmentsStore.dialogDataFieldMode) {
    case 'create':
      emit('create', {
        name: name.value,
